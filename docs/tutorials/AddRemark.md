...
This command allows users of BookKeeper application to add optional remarks to people in their bookkeeper and edit it if
required. The command should have the following format:
...
/**
 * Changes the remark of an existing client in bookkeeper.
 */
...
in-memory storage of client data. We achieve that by working with the `Person` model. Each field in a Person is
implemented as a separate class (e.g. a `Name` object represents the client’s name). That means we should add a `Remark`
class so that we can use a `Remark` object to represent a remark given to a client.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.client`. Since a `Remark` is a field that is similar to `Address`, we can
...
to [`seedu.address.ui.ClientCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
:exclamation: You must delete BookKeeper’s storage file located at `/data/bookkeeper.json` before running it! Not doing
so will cause BookKeeper to default to an empty bookkeeper!
